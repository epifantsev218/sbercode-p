...
openssl req -config ca.cnf -key private/ca.key.pem -new -x509 -days 7300 -sha256 -extensions v3_ca -out certs/ca.cert.pem -passout pass:qwe123 -passin pass:qwe123 -subj "/C=RU/ST=Moscow/L=Moscow/O=Sberbank/CN=test-ca"
...
openssl req -config intermediate/intermediate.cnf -new -sha256 -key intermediate/private/intermediate.key.pem -out intermediate/csr/intermediate.csr.pem -passout pass:qwe123 -passin pass:qwe123 -subj "/C=RU/ST=Moscow/L=Moscow/O=Sberbank/CN=test-intermediate"
